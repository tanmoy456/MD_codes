...
module parameters
    ...
    real*8, parameter   :: dt = 0.001, tf = 100.0 !time step & total time
    ...
    real*8             :: fs,PE,KE ,steps
    real*8             :: strt_time,end_time
end module parameters
program md
    use parameters
    call cpu_time(strt_time)  !for calculation of program runtime
    open(11,file='sc_initial.dat')
    !open(11,file='fcc_initial.dat')
    open(33,file='energy_MD.dat')
    open(44,file='final_pos_MD.txt')
    open(55,file='velocity_MD.txt')
    ...
    do while(t <= tf)
        ...
        call PBC
        steps = t/dt
        write(33,*)steps,KE/float(n_part),PE/float(n_part),(PE+KE)/float(n_part) 
